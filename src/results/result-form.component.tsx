      ...
      .then(
        (res) => {
          showSnackbar({
            ...
            kind: 'success',
            subtitle: t('labResultsUpdatedSuccessfully', 'Lab test results updated successfully'),
          });
          ...
          setFulfillerStatus(order.uuid, 'COMPLETED', abortController).then(
            () => {
              showSnackbar({
                ...
                kind: 'success',
                subtitle: t('testOrderCompletedSuccessfully', 'You have successfully completed the test order'),
              });

              mutate(
                (key) =>
                  typeof key === 'string' &&
                  key.startsWith(`${restBaseUrl}/order?orderTypes=${laboratoryOrderTypeUuid}`),
                undefined,
                { revalidate: true },
              );
            },
            (err) => {
              showNotification({
                title: t('errorMarkingOrderFulfillStatus', 'Error occurred while marking order fulfill status'),
                kind: 'error',
                critical: true,
  ...
  return (
    <Form className={styles.form}>
      ...
      <Stack className={styles.container} gap={5}>
        ...
        {concept && <ResultFormField register={register} concept={concept} control={control} />}
      ...
      <ButtonSet className={isTablet ? styles.tablet : styles.desktop}>
        <Button className={styles.button} onClick={closeOverlay} kind="secondary">
          {t('cancel', 'Cancel')}
        </Button>
